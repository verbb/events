...
{% if craft.events.isTicket(item) %}
    ...
    {% for purchasedTicket in ticket.getPurchasedTickets(item) %}
        Ticket UID: {{ purchasedTicket.uid }}<br />
    {% endfor %}
{% endif %}
```

:::tip
Because multiple tickets can be purchased for one line item, It's important to loop through potentially multiple unique ticket UID's as above.
:::

If you're looking for something a little prettier than simply showing the ticket UID in your order confirmation page or email, you may want to look at generating a [PDF Template](docs:template-guides/pdf-template).