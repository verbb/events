<?php
namespace Events\License;

use function Craft\craft;
use Craft\DateTimeHelper;
use Craft\EtException;
use Craft\LogLevel;
use Craft\Events_LicenseModel;
use Craft\EventsPlugin;

class Events_License
{
    private $etConnectFailureKey = 'connectFailure';
    private $etRecentPhoneHome = 'phonedHome';
    ...
    public function __construct($endpoint, $product, $productVersion, $licenseKey = '')
    {
        ...
        $this->_model = new Events_LicenseModel($attributes);
        ...
        try {
            ...
            if (!craft()->cache->get($this->etConnectFailureKey)) {
                ...
                if ($response->isSuccessful()) {
                    ...
                    $etModel = craft()->events_license->decodeEtModel($response->getBody());

                    if ($etModel) {
                        return $etModel;
                    } else {
                        EventsPlugin::log('Error in calling ' . $this->_endpoint . ' Response: ' . $response->getBody(), LogLevel::Warning);
            ...
            EventsPlugin::log('Error in ' . __METHOD__ . '. Message: ' . $e->getMessage(), LogLevel::Error);
