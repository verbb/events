...
class m190420_000000_craft3_version extends Migration
{
    public function safeUp(): bool
    {
        ...
        if ($this->db->tableExists('{{%events_eventtypes_i18n}}')) {
            ...
            Db::dropAllForeignKeysOnTable('{{%events_eventtypes_i18n}}');
            Db::dropAllIndexesOnTable('{{%events_eventtypes_i18n}}');

            // Drop the old locale FK column and rename the new siteId FK column
            $this->dropColumn('{{%events_eventtypes_i18n}}', 'locale');
            Db::renameColumn('{{%events_eventtypes_i18n}}', 'locale__siteId', 'siteId', $this);
        ...
        if ($this->db->tableExists('{{%events_eventtypes_i18n}}')) {
            Db::renameTable('{{%events_eventtypes_i18n}}', EventTypeSiteRecord::tableName(), $this);
            Db::renameColumn(EventTypeSiteRecord::tableName(), 'urlFormat', 'uriFormat', $this);
        }

        if ($this->db->columnExists('{{%events_tickets}}', 'ticketTypeId')) {
            Db::renameColumn('{{%events_tickets}}', 'ticketTypeId', 'typeId', $this);
        }
